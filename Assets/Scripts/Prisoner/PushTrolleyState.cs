...
public class PushTrolleyState : BaseStatePrisoner
{
    ...
    public override void Enable()
    {
        ...
        var finishPoint = _trolley.MovePoints
            .OrderBy(point => Vector2.Distance(prisonerStateMachine.Tr.position, point.position)).Last();
        StartCoroutine(MoveObjectToPosition(_trolley.Tr, finishPoint.position, _moveDuration));
        StartCoroutine(MoveObjectToPosition(prisonerStateMachine.Tr,
            new Vector2(finishPoint.position.x + closestHandlePos.localPosition.x, 0), _moveDuration));
    }
}