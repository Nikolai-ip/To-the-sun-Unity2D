...
namespace AISystem
{
    public class ThrowReactionState:BaseState
    {
        ...
        {
            _playerOriginPos = stateMachine.Player.transform.position;
            ...
            await Stay();
            var dir = stateMachine.Player.transform.position - stateMachine.Tr.position;
        ...
        private IEnumerator RunToThrowPosition(float dirX)
        {
            ...
            while (Math.Abs(_playerOriginPos.x - stateMachine.Tr.position.x) > 0.1f)
            {
                yield return null;
            }
